...
* Select _Product_ and enter a product name. The product name is not displayed yet.
...
### Development
Call ```go run cmd/coverage/main.go``` to start the server, including UI. But the Vue 3 app can also be started with ```npm run serve``` from the ```ui``` directory and is available on port 8081.
