...
RUN go mod download
# Copy built UI assets to the location expected by go:embed
# The Go code in api/ui/ui.go expects dist/ relative to itself.
# WORKDIR is /app/api, so we copy to ui/dist
