<template>
  <div class="container">
    ...
    <div
      ...
      :key="area['id']"
      :id="`area-${area['id']}`"
      class="area shadow p-2 mb-2 rounded"
    >
      <div class="row">
        ...
        <div class="col-5 mb-2">
          ...
          <span v-if="area['total'] > 0" class="result total">
            {{ area['total'] }}
            <i
              v-if="area['total'] > area['first-total']"
        ...
        <div class="col mb-2">
          <span
            class="result expl-test pointer"
            @click="showExplTests(area['id'])"
          >
            {{ parseFloat(area['expl-rating']).toFixed(1) }} ({{
              area['expl-tests']
            }})
          </span>
          &nbsp;
  ...
  <div
    ...
    id="logExplTest"
    tabindex="-1"
    aria-labelledby="logExplTestLabel"
    aria-hidden="true"
  >
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="logExplTestLabel">
            Log Exploratory Test
          </h5>
          <button
            type="button"
            class="btn-close"
  ...
  <div
    ...
    id="showExplTest"
    tabindex="-1"
    aria-labelledby="showExplTestLabel"
    aria-hidden="true"
  >
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form>
          <div class="modal-body">
            <div v-for="et in explTests" :key="et['id']">
              <label>
                <strong
                  >{{ new String(et['test-run']).split('T')[0] }} /
                </strong>
                <i
                  class="bi"
                  v-for="n in 5"
...
export default defineComponent({
  ...
  methods: {
    async getAreas() {
      this.loading = true;
      await fetchData(`/api/v1/coverage/${this.productId}/areas`)
        .then((data) => {
          this.areas = data;
        })
        .catch((err) => {
    ...
    async saveExplTest() {
      await fetchData(`/api/v1/expl-tests`, {
        method: 'POST',
        mode: 'cors',
        body: JSON.stringify({
    ...
    async showExplTests(areaId: number) {
      await fetchData(`/api/v1/expl-tests/area/${areaId}`)
        .then((data) => {
          this.explTests = data;
        })
        .catch((err) => {
